...
class pointsAB{
    ...
    public function getFullname($a,$b){
        ...
        if ($y->rowCount() >= 1) {
            ...
            $this->firstStage($y1['tail'],$y1['ver']);
        }
       return $this->xcount;
    }

    private function firstStage($a,$b){
...
function  packPoints($a,$b){
    ...
    if($x->rowCount()>=1){
       while($x1=$x->fetch()){
            packPoints1($x1['fullname'],$b,IdtoName($a)[0],$x1['join_date'],$x1['entry'],$x1['fullname']);
        }
    }
}
function  packPoints1($name,$Cid,$a,$jd,$entry,$memFname){
    $aa=strtoupper($a);
    ...
    if($y->rowCount()>=1){
        if($y1=$y->fetch()){
            ...
            if($head==$aa && $posx==$Cid){
                ...
                if($GLOBALS['limitCH']!=5){
                packPoints1($y1['head'],$Cid,$a,$jd,$entry,$memFname);
                }
            }
        }
    }
