...
{
    if(query("SELECT code5 FROM earnings WHERE client=$a1 AND code5>0")){
        ...
        if ($x->rowCount())
        {
           ...
           if($x2['entry']==5){
                ...
                if (query("UPDATE `clients` SET `entry` = 5 WHERE `id` = $a1") >= 1)
                {
                    send_ref($a, $a2,$a1);
                }
            } 

            query("UPDATE `earnings` SET `code5`=`code5`-1 WHERE `client`=$a1");
            echo "success";
        }
    }else{
        echo 'success';
